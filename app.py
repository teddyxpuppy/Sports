...
def select_academy():
    ...
    # Fetch any additional information about the selected academy if necessary
    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT * FROM academies WHERE id = %s", (academy_id,))
    academy = cursor.fetchone()
    cursor.close()

    # Store the selected academy in the session or handle it as needed
    session['selected_academy'] = academy

    # Redirect to sport.html page
    return redirect(url_for('sport_page'))  # Redirecting to sport_page function
# Add this route to your existing code
...
def sport_page():
    ...
    # Pass the selected academy details to the template if needed
    return render_template('sport.html', academy=selected_academy)
